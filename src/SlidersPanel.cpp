...
void SlidersPanel::update()
{
	if (mVisible && !mParameterBag->mOptimizeUI)
	{
		...
		sliderZPos->setName("z Pos:" + toString(floor(mParameterBag->mZPosition)));
...
void SlidersPanel::tempoZPos(const bool &pressed)
{
	tZPos = pressed;
	if (!pressed) resetZPos(pressed);
}
void SlidersPanel::resetZPos(const bool &pressed)
{
	autoZPos = false;
	tZPos = false;
